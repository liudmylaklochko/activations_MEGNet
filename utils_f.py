...
def return_tuple(activations, dataloader, model, list_layers,count):
    t=()
    t = t + (activations, )
    t = t + (dataloader, )
    t = t + (model,)
...
def get_first_linear_out_features(module):
    import torch.nn as nn
    last_out = None
    for child in module.children():
        out = get_first_linear_out_features(child)
        if out is not None:
            last_out = out
    if isinstance(module, nn.Linear):
        last_out = module.out_features
    return last_out
...
def plot_ICE(df,perturb_values,feature_index,layer,config):
    filename = f"acts_{layer}.pt"
    activation_data = torch.load(config['actdir']+"/acts_"+layer+".pt")
    decoded_activations = torch.load(config['saedir']+"/decoded_"+layer+".pkl")
