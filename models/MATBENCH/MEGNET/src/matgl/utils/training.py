...
class MatglLightningModuleMixin:
    ...
    def validation_step(self, batch: tuple, batch_idx: int):
        results, batch_size, preds = self.step(batch)
        self.validation_preds.append(preds.detach().cpu())  # avoid keeping computation graph
        return {"preds": preds}

    def on_validation_epoch_end(self):
        self.all_validation_preds = torch.cat(self.validation_preds, dim=0)
        print("All preds collected:", self.all_validation_preds)
...
class ModelLightningModule(MatglLightningModuleMixin, pl.LightningModule):
    ...
    def step(self, batch: tuple):
        ...
        return  results, batch_size, preds
